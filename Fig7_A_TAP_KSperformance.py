...
for i, r in enumerate(rates):
    ...
    # ====== 在这里添加代码 ======
    print(f"\n{'=' * 60}")
    print(f"子图 {sub_labels[i]}: Rate = {r}s 的所有数据点")
    print(f"{'=' * 60}")
    for model in models_comparison:
        ...
        if not model_data.empty:
            ...
            for _, row in model_data.iterrows():
                print(f"  P = {int(row['Phis'])}s: KS Distance = {row['tes_KS_Dist']:.6f}")
    print(f"{'=' * 60}")
    # ====== 添加结束 ======
    ...
    ax.text(0.5, -0.32, f'{sub_labels[i]} Rate = {r}',
...
# 保存图片
plt.savefig('Figure_1_KS_Analysis_Final.pdf', dpi=300, bbox_inches='tight')
plt.show()